...
public class TicTacToe {
	...
	public boolean check_if_winning(INDArray bool_filled){
		...
		if (num_dim == 3){
			...
			int[] max_index = {2, 2, 2};
			int[] middle_index = {1, 1, 1};
			
			int diag_trace = filled.getInt(start_index) + filled.getInt(middle_index) + filled.getInt(max_index);
			is_winning = diag_trace == winning;
			
			for(int i=0; i<num_dim; i++){
				start_index = new int[]{0, 0, 0};
				start_index[i] = 2;
				max_index = new int[]{2, 2, 2};
				max_index[i] = 0;
				
				diag_trace = filled.getInt(start_index) + filled.getInt(middle_index) + filled.getInt(max_index);
...
//		int 
		...
		float avr = totalTries/100000;
		float percent = counter /100000;
		...
		//initialize players;
		Player player1 = new Player(1, game.mode, game.probability, game);
		Player player2 = new Player(2, game.mode, game.probability, game);
